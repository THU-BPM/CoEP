...
class Generator(object):
    ...
    def predictor(self, data, none_token_id=39763):
    ...
    def generate(self, data, max_length, num_beams, num_return_sequences=1, length_penalty=None,
        ...
        for step, batch in enumerate(data):
            batch = tuple(t.to(self.device) for t in batch)
            with torch.no_grad():
                input_ids, input_mask, segment_ids, label_ids, labels_masks, _ = batch
        ...
        if "csv" in save_file:
            cols = ["predicted_refs", "test_refs"]
            df = pd.DataFrame(columns=cols)
            df["predicted_refs"] = all_generated_sents
            df["test_refs"] = all_label_sents
            df.to_csv(save_file, index=True)
            self.logger.info(f"save sentences in csv files:{save_file}")
        elif "pkl" in save_file:
            import pickle
            ...
            self.logger.info(f"invalid file format in {save_file}")
