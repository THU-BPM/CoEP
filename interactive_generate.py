...
from src.test.generator_CoEP_interactive import Generator

from src.model.bart_for_generation import BartWithClassification
from src.model.CoEP_Model import CoEP


def load_atomic_KG_model(n_gpu):
    resume_path = ft_config["atomic_kg_wcls_model"]
    IM = BartWithClassification(path=config['bart-base_model_dir'], num_labels=2)
...
def main():
    ...
    parser.add_argument('--model_path', default='output/checkpoints/bart-base_event_story_CoEP_2e-5/'
    ...
    coep = CoEP(kg_bart_model=kg_model.model, bart_model=GM, num_labels=2)
    if args.model_path is None:
        coep.load_state_dict(torch.load(ft_config['CoEP'], map_location=f"cuda:{args.n_gpu}")['model'])
    else:
        coep.load_state_dict(torch.load(args.model_path, map_location=f"cuda:{args.n_gpu}")['model'])
    ...
    generator = Generator(model=coep, tok=tok, logger=logger, n_gpu=args.n_gpu)
